...
// function to pause execution of game for a given amount of time
...
// function to show the path of colors to the player
...
const handleColorClick = async (e) => {
    // if the path is current being generated, ignore click
    if (isPathGenerating) {
        return false;
    }

    // if clicked color is correct, update the score and continue generating the path
    if (e.target.classList.contains(randomColors[clickCount])) {
        ...
        // if the clicked color is incorrect, end the game
    } else {
        endGame();
    }
};
